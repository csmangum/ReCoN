...
def setup_recon_network():
    ...
    if st.session_state.recon_graph is None:
        try:
            # Compile the graph
            st.session_state.recon_graph = compile_from_file('/workspace/scripts/engage_active_perception.yaml')
...
def generate_simulated_audio():
    ...
    for key in st.session_state.audio_features:
        st.session_state.audio_features[key] = np.clip(st.session_state.audio_features[key], 0.0, 1.0)
...
def step_network():
    ...
    if st.session_state.recon_engine:
        ...
        active_units = [u for u in st.session_state.recon_graph.units.values() if u.state in [State.ACTIVE, State.TRUE, State.CONFIRMED]]
        confirmed_units = [u for u in st.session_state.recon_graph.units.values() if u.state == State.CONFIRMED]
        ...
        if len(st.session_state.activation_history) > 100:
            st.session_state.activation_history = st.session_state.activation_history[-100:]
...
def create_network_graph():
    ...
    for edge in G.edges():
        ...
        edge_info.append(f"Type: {edge_data['type']}<br>Weight: {edge_data['weight']:.2f}")
    ...
    fig.update_layout(
        title="ReCoN Network: 'Engage Active Perception'",
        titlefont_size=16,
        showlegend=True,
        hovermode='closest',
...
def create_network_layout():
    ...
    terminals = [uid for uid, u in st.session_state.recon_graph.units.items() if u.kind == UnitType.TERMINAL]
...
def main():
    ...
    st.subtitle("'Engage Active Perception' Hypothesis")
    ...
    with st.expander("Network Information"):
        if st.session_state.recon_graph:
            ...
            # Count units by type
            script_units = [u for u in st.session_state.recon_graph.units.values() if u.kind == UnitType.SCRIPT]
            terminal_units = [u for u in st.session_state.recon_graph.units.values() if u.kind == UnitType.TERMINAL]
