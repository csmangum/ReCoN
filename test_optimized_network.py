#!/usr/bin/env python3
"""
Test the Optimized ReCoN Network with Balanced Sensitivity
"""
...
from perception.optimized_continuous_audio_terminals import OptimizedContinuousAudioProcessor


def test_optimized_network(audio, sr, description, expected_result):
    """Test optimized network performance and return detailed results."""
    
    print(f"\n🔍 Testing Optimized Network: {description}")
    print(f"   Expected: {expected_result}")
    print("=" * 60)
    
    # Initialize optimized ReCoN network
    try:
        graph = compile_from_file("scripts/optimized_continuous_syllable_listener.yaml")
        engine = Engine(graph)
        processor = OptimizedContinuousAudioProcessor()
    except Exception as e:
        print(f"  ❌ Error initializing optimized network: {e}")
        return None
    ...
    key_units = {
        ...
        'u_music_detector': 'Music Detection',
    ...
    if expected_result == "MATCH":
        ...
        success = (speaking and 
                  ...
                  'u_noise_detector' not in confirmed_units and
                  'u_music_detector' not in confirmed_units)
        ...
        success = (not speaking or 
                  ...
                  'u_noise_detector' in confirmed_units or
                  'u_music_detector' in confirmed_units)
        result_icon = "✅" if success else "❌"
...
def create_test_audio():
    ...
    # PATTERN: Matching audio (hello) - improved
    print("  Creating matching pattern: 'hello' (improved)")
    ...
    sf.write("test_patterns/hello_improved.wav", hello_audio, sample_rate)
    ...
    # ANTI-PATTERN: Music chord (improved)
    print("  Creating anti-pattern: music chord (improved)")
    t = np.linspace(0, 1.0, int(1.0 * sample_rate))
    chord_audio = (0.3 * np.sin(2 * np.pi * 261.63 * t) +  # C
                   0.3 * np.sin(2 * np.pi * 329.63 * t) +  # E
                   0.3 * np.sin(2 * np.pi * 392.00 * t))   # G
    sf.write("test_patterns/chord_improved.wav", chord_audio, sample_rate)
...
def create_synthetic_syllable_audio(syllable: str, sample_rate: int, duration: float) -> np.ndarray:
    ...
    if syllable == "hɛ":  # "he"
        # /h/ - fricative
        h_audio = np.random.normal(0, 0.15, len(t)) * np.exp(-t * 1.5)
        # /ɛ/ - low front vowel
        e_audio = (0.5 * np.sin(2 * np.pi * 600 * t) +
        ...
        # /l/ - lateral
        l_audio = (0.4 * np.sin(2 * np.pi * 800 * t) +
                   0.3 * np.sin(2 * np.pi * 1600 * t))
        # /oʊ/ - diphthong
        t1 = t[:len(t)//2]
        t2 = t[len(t)//2:]
        o_audio = (0.5 * np.sin(2 * np.pi * 500 * t1) +
                   0.4 * np.sin(2 * np.pi * 1000 * t1))
        u_audio = (0.5 * np.sin(2 * np.pi * 400 * t2) +
                   0.4 * np.sin(2 * np.pi * 800 * t2))
...
def main():
    """Test optimized network performance."""
    
    print("🚀 Testing Optimized ReCoN Network with Balanced Sensitivity")
    print("=" * 70)
    ...
    test_cases = [
        ...
        ("test_patterns/hello_improved.wav", "Matching Pattern: 'hello' (improved)", "MATCH"),
        ...
        ("test_patterns/chord_improved.wav", "Anti-Pattern: Music Chord (improved)", "REJECT"),
    ...
    for file_path, description, expected in test_cases:
        if os.path.exists(file_path):
            ...
            # Test network
            result = test_optimized_network(audio, sr, description, expected)
            if result:
                results.append({
                    'file': file_path,
    ...
    # Summary analysis
    print(f"\n📊 OPTIMIZED NETWORK PERFORMANCE SUMMARY")
    print("=" * 70)
    ...
    print(f"  ✅ Lowered syllable thresholds for better pattern recognition")
    print(f"  ✅ Added music detection with harmonic analysis")
    print(f"  ✅ Balanced sensitivity between detection and rejection")
    print(f"  ✅ Improved feature discrimination and temporal constraints")
